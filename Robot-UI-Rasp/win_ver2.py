        ...
        try:
            ...
            if bytetoread > 0:
                ...
                if msg.startswith("addroom:"):
                    ...
                    arrive_goal()
                    # msg.startswith("arrivedgoal"):
                    # _, station = msg.split(":")
                    # insertTextBox(textDashBoard, "\nYour delivery has arrived at {} \n".format(station))
                elif msg == "robot:on":
                    insertTextBox(textDashBoard, "\nYour delivery has arrived \n")
                    ...
                    if switch == "on":
                        web = 2
                        messagebox.showinfo("Web Admin", "Web Admin is on")
                        insertTextBox(textDashBoard, "\nWeb Admin is on\n")
                    else:
                        web = 0
                        messagebox.showinfo("Web Admin", "Web Admin is off")
                        insertTextBox(textDashBoard, "\nWeb Admin is off\n")
...
def draw_buttons():
    # Xóa toàn bộ widget cũ trong khung chứa button_frame
    for widget in middle_frame.winfo_children():
        widget.destroy()
        
    # Vẽ lại các widget trong khung chứa button_frame
    col = 0
    row = 0
    for _, button_name in enumerate(room_list):
        ...
        button.grid(row=row, column=col, pady=10, padx=2)
...
# setup các function cho right button
def confirm_send():
    if len(selected_rooms) == 0:
        messagebox.showerror("Error", "No room is selected.")
        return
    rooms = "\n".join(selected_rooms)
    confirm = messagebox.askokcancel("Confirm", "Do you want to send " + str(len(selected_rooms)) + " selected rooms ?\n" + rooms)
    if confirm:
        send_room_list()
...
def popup():
    ...
    # RobotService = tk.Button(popup_window, text="Robot Service", command=lambda: (robot_switch(), popup_window.destroy()), width=BUTTON_WIDTH*2, height=BUTTON_HEIGHT)
    # RobotService.grid(row=1, column=0, padx=2, pady=10)
    
    # Web = tk.Button(popup_window, text="Web", command=lambda: (web_switch(), popup_window.destroy()), width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
    # Web.grid(row=1, column=1, padx=2, pady=10)
    
    # exit_button = tk.Button(popup_window, text="Exit", command=popup_window.destroy, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
    # exit_button.grid(row=1, column=2, padx=2, pady=10)
    ...
    # button_width = int(0.125 * popup_width)
    # button_height = int(0.1042 * popup_height)
...
# popup_width = int(0.625 * root.winfo_screenwidth())
# popup_height = int(0.4167 * root.winfo_screenheight())
